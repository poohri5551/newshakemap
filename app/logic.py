...
def _thai_region_from_epicenter(
    lat: float,
    lon: float,
    ...
    province = _normalize_thai_province_name(changwat)

    # 1) ตัดสินจากชื่อจังหวัดก่อน
    ...
    # 2) ถ้าไม่มี changwat ลองดูจากข้อความ region_text
    txt = (region_text or "").strip()
    for p in NORTH_PROVINCES:
        if p in txt:
            return "north"
    for p in WEST_PROVINCES:
        if p in txt:
            return "west"
    ...
    # 3) fallback ด้วย lat/lon
    if lat < 11.0:
        return "south"

    # บีบกรอบ west ให้แคบลงกว่าของเดิม
    if 12.0 <= lat <= 18.8 and 98.3 <= lon <= 99.2:
        return "west"
...
def _get_mmi_model_for_region(
    ...
    region = _thai_region_from_epicenter(
        lat=lat,
        lon=lon,
        changwat=changwat,
...
def compute_overlay_from_event(ev: dict):
    ...
    grid_model, grid_model_region = _get_mmi_model_for_region(
        lat,
        lon,
        changwat=changwat,
        region_text=region_text,
    ...
    print(
        f"[MMI_GRID] selected region={grid_model_region} "
    ...
    if grid_model is not None:
        try:
            ...
            mmi_pred_grid = pred_guarded
            mmi_level_grid = np.clip(np.rint(pred_guarded), 1, 12).astype(int)
